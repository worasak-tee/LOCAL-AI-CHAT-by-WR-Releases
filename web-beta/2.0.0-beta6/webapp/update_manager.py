...
def _fetch_manifest() -> dict[str, Any]:
    ...
    request = urllib.request.Request(
        MANIFEST_URL,
        headers={"User-Agent": "LOCAL-AI-CHAT-by-WR-Web-Updater/1.3", "Accept": "application/json"},
    )
...
def _install_optional_extensions(app, ctx: dict[str, Any]) -> None:
    ...
    chat_files_dir = Path(ctx.get("CHAT_FILES_DIR") or (Path(settings.data_dir) / "chat_files"))
    chat_files_dir.mkdir(parents=True, exist_ok=True)
    ctx["CHAT_FILES_DIR"] = chat_files_dir
    ...
    if errors:
        ...
        ctx.pop("_wr_optional_extension_errors", None)
